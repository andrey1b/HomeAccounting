...
{
    public Income Result { get; private set; } = new();
    public bool OpenAnother { get; private set; }

    public IncomeDialog(Income? existing = null)
    {
        InitializeComponent();
        ApplyLoc();

        var accounts   = AccountService.GetAll(true);
        var categories = CategoryService.GetAll("income");
    ...
    {
        Title                 = AppLoc.T("dlg_income_title");
        TbLblDate.Text        = AppLoc.T("lbl_date");
        TbLblAccount.Text     = AppLoc.T("lbl_account_to");
