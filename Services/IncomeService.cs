...
public static class IncomeService
{
    ...
    public static Income? GetById(int id)
    {
        ...
        return conn.QueryFirstOrDefault<Income>(
            "SELECT id,date,account_id,category_id,subcategory_id,quantity,unit_id,amount,note FROM incomes WHERE id=@id",
            new { id });
    }
    ...
    public static int Add(Income i)
    {
        ...
        return conn.ExecuteScalar<int>(@"
            INSERT INTO incomes(date,account_id,category_id,subcategory_id,quantity,unit_id,amount,note)
            VALUES(@d,@a,@c,@s,@q,@u,@am,@n);
    ...
    public static void Update(Income i)
    {
        ...
        conn.Execute(@"
            UPDATE incomes SET date=@d,account_id=@a,category_id=@c,subcategory_id=@s,
                quantity=@q,unit_id=@u,amount=@am,note=@n WHERE id=@id",
            new { d=i.Date.ToString("yyyy-MM-dd"), a=i.AccountId, c=i.CategoryId,
                  s=i.SubcategoryId, q=i.Quantity, u=i.UnitId, am=i.Amount, n=i.Note, id=i.Id });
    }

    public static void Delete(int id)
    {
        using var conn = Db.Open();
        conn.Execute("DELETE FROM incomes WHERE id=@id", new { id });
    }
}
