...
public static class AppLoc
{
    ...
    private static readonly Dictionary<string, string[]> D = new()
    {
        ...
        ["tab_shopping"] = ["Список покупок",     "Список покупок",     "Shopping list"],
        ["shop_all"]     = ["Вибрати все",        "Выбрать все",        "Select all"],
        ...
        ["shop_scan_hint"]   = ["Відскануйте QR-код камерою телефона або відкрийте адресу в браузері:",
                                "Отсканируйте QR-код камерой телефона или откройте адрес в браузере:",
                                "Scan the QR with your phone camera or open the address in a browser:"],
        ["shop_empty"]   = ["Не відмічено жодного товару.", "Не отмечено ни одного товара.", "No items selected."],
        ["shop_count"]   = ["Товарів у списку: {count}. Відмітьте потрібні, вкажіть кількість і натисніть «Відкрити на телефоні».",
                            "Товаров в списке: {count}. Отметьте нужные, укажите количество и нажмите «Открыть на телефоне».",
                            "Items: {count}. Tick the ones you need, set quantity, then «Open on phone»."],
        ...
        ["ai_tab"]        = ["Запитати ШІ",   "Спросить у ИИ",  "Ask AI"],
        ["ai_question"]   = ["Питання:",      "Вопрос:",        "Question:"],
        ["ai_ask"]        = ["▶  Запитати",   "▶  Спросить",    "▶  Ask"],
        ["ai_save_all"]   = ["Зберегти все",  "Сохранить все",  "Save all"],
        ["ai_clear"]      = ["Очистити",      "Очистить",       "Clear"],
        ["ai_api_keys"]   = ["⚙ API ключі",   "⚙ API ключи",    "⚙ API keys"],
        ["ai_save"]       = ["Зберегти",      "Сохранить",      "Save"],
        ["ai_copy"]       = ["Копіювати",     "Копировать",     "Copy"],
        ["ai_copied"]     = ["Скопійовано в буфер", "Скопировано в буфер", "Copied to clipboard"],
        ["ai_quick"]      = ["Швидкі питання:", "Быстрые вопросы:", "Quick questions:"],
        ["ai_qb_save"]    = ["Як заощадити",    "Как сэкономить",   "Save money"],
        ["ai_qb_plan"]    = ["План накопичень", "План накоплений",  "Savings plan"],
        ["ai_qb_budget"]  = ["Правило 50/30/20","Правило 50/30/20", "50/30/20 rule"],
        ["ai_q_save"]     = ["Як заощадити на щомісячних витратах сім'ї? Дай практичні поради.",
                             "Как сэкономить на ежемесячных расходах семьи? Дай практичные советы.",
                             "How can a family save on monthly expenses? Give practical tips."],
        ["ai_q_plan"]     = ["Склади простий покроковий план накопичень для сімейного бюджету.",
                             "Составь простой пошаговый план накоплений для семейного бюджета.",
                             "Make a simple step-by-step savings plan for a family budget."],
        ["ai_q_budget"]   = ["Поясни просто правило бюджету 50/30/20 і як його застосувати.",
                             "Объясни просто правило бюджета 50/30/20 и как его применить.",
                             "Explain the 50/30/20 budgeting rule simply and how to apply it."],
        ["ai_empty_title"]= ["Питання порожнє", "Вопрос пуст",    "Empty question"],
        ["ai_empty_msg"]  = ["Введіть питання.", "Введите вопрос.", "Enter a question."],
        ["ai_nothing_msg"]= ["Немає відповіді для збереження.", "Нет ответа для сохранения.", "No answer to save."],
        ["ai_status_wait"]= ["⌛ Чекаю відповіді ШІ…", "⌛ Жду ответы от ИИ…", "⌛ Waiting for AI…"],
        ["ai_status_done"]= ["✓ Готово!",     "✓ Готово!",      "✓ Done!"],
        ["ai_status_browser"]=["🌐 Відкрито в браузері: {list}", "🌐 Открыт(ы) в браузере: {list}", "🌐 Opened in browser: {list}"],
        ["ai_status_none"]= ["Немає вибраних ШІ.", "Нет выбранных ИИ.", "No AI selected."],
        ["ai_browser_note"]=["🌐 Питання відкрито в браузері.\nПитання скопійовано в буфер — вставте (Ctrl+V) у чат.\nСкопіюйте відповідь сюди після отримання.",
                             "🌐 Вопрос открыт в браузере.\nВопрос скопирован в буфер — вставьте (Ctrl+V) в чат.\nСкопируйте ответ сюда после получения.",
                             "🌐 Question opened in the browser.\nIt is copied to the clipboard — paste (Ctrl+V) into the chat.\nCopy the answer back here."],
        ["ai_req_to"]     = ["⌛ Запит до {name}…", "⌛ Запрос к {name}…", "⌛ Request to {name}…"],
        ["ai_err"]        = ["❌ Помилка",   "❌ Ошибка",      "❌ Error"],
        ["ai_stats"]      = ["Символів: {chars} | Слів: {words}", "Символов: {chars} | Слов: {words}", "Chars: {chars} | Words: {words}"],
        ["ai_save_header"]= ["Відповіді ШІ", "Ответы ИИ",      "AI answers"],
        ["ai_keys_title"] = ["API ключі для ШІ", "API ключи для ИИ", "API keys for AI"],
        ["ai_keys_save"]  = ["Зберегти",     "Сохранить",      "Save"],
        ["ai_key_claude_link"]    = ["Отримати на console.anthropic.com", "Получить на console.anthropic.com", "Get it at console.anthropic.com"],
        ["ai_key_gemini_link"]    = ["Безкоштовно на aistudio.google.com", "Бесплатно на aistudio.google.com", "Free at aistudio.google.com"],
        ["ai_key_deepseek_link"]  = ["Отримати на platform.deepseek.com", "Получить на platform.deepseek.com", "Get it at platform.deepseek.com"],
        ["ai_key_perplexity_link"]= ["Отримати на perplexity.ai/settings/api", "Получить на perplexity.ai/settings/api", "Get it at perplexity.ai/settings/api"],
